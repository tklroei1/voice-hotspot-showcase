...
- **Per-speaker baselining, live.** The product builds a rolling baseline per speaker over the call and asks "is *this* turn more activated than *their* usual?" — the normalization that lifts shipped accuracy to 83.7%.
...
Speaker-independent split on **RAVDESS** (train actors 1–18, test 19–24). Binary **arousal** is the coaching-critical signal:

| Approach | Test accuracy |
|---|---|
| Hand-weighted heuristic | 65.7% |
| Logistic regression + prosodic/spectral features | 81.7% (F1 85.1%) |
| **+ per-speaker normalization (shipped)** | **83.7%** |
| + spectral + MFCC (MLP) | 86.2% (F1 88.4%) — documented next increment |

Full methodology and scripts live with the private source; MELD is used to sanity-check the text layer.
...
*Voice Hotspot · built by [tklroei1](https://github.com/tklroei1). Source is private — reach out for a walkthrough or access.*
